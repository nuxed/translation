...
final class MessageCatalogue {
  ...
  public function __construct(
    string $locale,
    KeyedContainer<string, KeyedContainer<string, string>> $messages = dict[],
  ) {
    $this->locale = $locale;
  ...
  public function getDomains(): Container<string> {
    $domains = vec[];
    foreach ($this->messages as $domain => $_messages) {
      $domains[] = $domain;
    }
    return $domains;
  }
  ...
  public function addFallbackCatalogue(MessageCatalogue $catalogue): void {
    ...
    while ($fallback is nonnull) {
      if ($fallback->getLocale() === $this->getLocale()) {
        throw new Exception\LogicException(Str\format(
    ...
    $parent = $this;
    do {
      if ($parent->getLocale() === $catalogue->getLocale()) {
        throw new Exception\LogicException(Str\format(
          'Circular reference detected when adding a fallback catalogue for locale "%s".',
          $catalogue->getLocale(),
        ));
